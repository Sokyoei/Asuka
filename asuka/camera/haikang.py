...
class HaikangCamera(threading.Thread):
    ...
    def open_stream(self):
        ...
        if not self.LoginDev():  # 登录设备
            return False
        if not self.startPlay(playTime=5):  # playTime用于linux环境控制预览时长，windows环境无效  # noqa: SIM103
            return False
        return True
    ...
    def LoginDev(self):
        ...
        if self.iUserID < 0:
            logger.error(f"Login failed, error code: {self.hikSDK.NET_DVR_GetLastError()}")
            self.hikSDK.NET_DVR_Cleanup()
            return False
        else:
    ...
    def startPlay(self, playTime):
        ...
        if self.lRealPlayHandle < 0:
            logger.info(f"Open preview fail, error code is: {self.hikSDK.NET_DVR_GetLastError()}")
            # 登出设备
            self.hikSDK.NET_DVR_Logout(self.iUserID)
            # 释放资源
            self.hikSDK.NET_DVR_Cleanup()
